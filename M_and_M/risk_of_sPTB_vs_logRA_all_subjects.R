##
## risk of sPTB vs log10 relAb's in all subjects over all visits
##
...
pics.dir <- "../pics/sPTB_dir/"
dir.create(pics.dir)

pr.sPTB.all.subjects.fn <- function(ph, nModels=5, nChains=3, nItr=2000, thin=1, nCores=3, showMessages=FALSE, picsDir=pics.dir)
{
    x <- ct[,ph]
    n <- rowSums(ct)
    y <- mt$delStatus
...
all.res <- mclapply(sel.25, pr.sPTB.all.subjects.fn, mc.cores=7)


all.gEff <- matrix(nrow=length(sel.25), ncol=8)
colnames(all.gEff) <- c("gEff", "p-val","q-val","n(TERM)","n(sPTB)", "n(subj TERM)","n(subj sPTB)","-mode($\\log_{10}$(RA))")
rownames(all.gEff) <- sel.25
for ( i in seq(sel.25) )
{
    r <- all.res[[i]]
    ph <- r$ph
