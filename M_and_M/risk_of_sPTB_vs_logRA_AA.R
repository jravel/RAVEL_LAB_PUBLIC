##
## risk of sPTB vs log10 relAb's in AA subjects over all visits
##
...
pr.sPTB.AA.fn <- function(ph, nModels=5, nChains=3, nItr=2000, thin=1, nCores=3, showMessages=FALSE, picsDir=pics.dir)
{
    x <- ct.AA[,ph]
    n <- rowSums(ct.AA)
    y <- mt.AA$delStatus
...
AA.res <- mclapply(sel.25, pr.sPTB.AA.fn, mc.cores=7)
