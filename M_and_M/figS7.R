...
##
## Mb :: Lb :: BD interaction in the context of sPTB at visit V1
##
...
x <- rowSums(ct.v1[idx, c(1,2,4,6)])
n <- rowSums(ct.v1[idx,])
x.Lb <- log10(x/n)
x.Mb <- log10(pt.v1[idx,ph])
x.bd <- log10(bdt.v1$BD_v1[idx])
x.cst <- mt.v1$cst[idx]
x.subjID <- mt.v1$subjID[idx]
...
file <- "../pics/Lb_vs_BD_vs_Mb_with_CSTs_with_no_CSTI_outlier.pdf"
...
file <- "../pics/Lb_vs_BD_vs_Mb_with_CSTs_with_no_CSTI_outlier_version_with_squares.pdf"
pdf(file, width=6, height=6)
op <- par(mar=c(3.75, 3.75, 4.5, 3.5), mgp=c(2.5,0.5,0),tcl = -0.3)
plot(x.Lb, x.bd, xlab="log10( relAb of Lactobacillus )",
     ylab="log10( BD )",
