##
## risk of sPTB34 vs log10 relAb's in AA subjects over all visits
##
...
idx <- mt$delStatus34 != 2 & !is.na(mt$raceChar) & mt$raceChar=="black"
mt.AA34 <- mt[idx,]
ct.AA34 <- ct[idx,]
pt.AA34 <- pt[idx,]

y <- mt.AA34$delStatus34
s <- mt.AA34$subjID
(tt <- table(y))
...
pics.dir <- "../pics/sPTB34_AA_dir/"
dir.create(pics.dir)

pr.sPTB.AA34.fn <- function(ph, nModels=5, nChains=3, nItr=2000, thin=1, nCores=3, showMessages=FALSE, picsDir=pics.dir)
{
    x <- ct.AA34[,ph]
    n <- rowSums(ct.AA34)
    y <- mt.AA34$delStatus34
...
AA34.res <- mclapply(sel.25, pr.sPTB.AA34.fn, mc.cores=7)


AA34.gEff <- matrix(nrow=length(sel.25), ncol=8)
colnames(AA34.gEff) <- c("gEff", "p-val","q-val","n(TERM)","n(sPTB)", "n(subj TERM)","n(subj sPTB)","-mode($\\log_{10}$(RA))")
rownames(AA34.gEff) <- sel.25
for ( i in seq(sel.25) )
{
    r <- AA34.res[[i]]
    ph <- r$ph
...
AA34.gEff[,3] <- p.adjust(AA34.gEff[,2], method="fdr")

o <- order(AA34.gEff[,2])
AA34.gEff <- AA34.gEff[o,]

q.thld <- 0.05
e.thld <- 0.1
idx <- AA34.gEff[,3] < q.thld & abs(AA34.gEff[,1]) > e.thld
(AA34.gEff.sig <- as.data.frame(AA34.gEff)[idx,])
