...
AA.cst.q <- matrix(NA,nrow=length(CSTs), ncol=5)
rownames(AA.cst.q) <- CSTs
colnames(AA.cst.q) <- c("0\\%","25\\%", "50\\%", "75\\%","100\\%")
for ( cst in CSTs )
{
...
write.csv(AA.cst.q, file="AA_cst_bd_quartiles.csv", quote=F)


## AA V1 phylotypes

AA.phs.q <- matrix(NA,nrow=length(phs), ncol=5)
rownames(AA.phs.q) <- phs
colnames(AA.phs.q) <- c("0\\%","25\\%", "50\\%", "75\\%","100\\%")
for ( ph in phs )
{
    idx <- ct.v1.AA[,ph] > 0
    y <- log10(bdt.v1.AA$BD_v1[idx])
...
write.csv(AA.phs.q, file="AA_phs_bd_quartiles.csv", quote=F)



AA.v1.cst.fn <- function(cst, nItr=50000, thin=5, nChains=4, nCores=4)
{
    idx <- mt.v1.AA$cst==cst
    y <- log10(bdt.v1.AA$BD_v1[idx])
    x <- mt.v1.AA$sPTB[idx]
...
tblTexFile <- "../docs/Tables/AA_q_bd_V1_CST.tex"
tblLabel <- "AA.q.bd.v1.cst:tbl"
...
file <- paste0(tblDir,"tbl6.csv")
write.csv(AA.bd.v1.cst.tbl, file=file, quote=F)
file
