...
load("spmrf_bernoulli_horseshoe_order2.rda")
...
m.2prop.fn <- function(m.dat, nItr=50000, thin=5, nChains=4, nCores=4)
{
    ...
    logRat.ci <- c(logRat.ci, max(c(logRat.pval, fn.pval)), logRat.pval, fn.pval)
                      ...
                      med.col="blue", bci.col="gray90")
{
    ...
    plot(x, y.med, type = "n", ylim=c(0,1), las=1,  xlab=xlab, ylab=ylab, main=title) # ylim=range(c(theta.l, theta.u))
...
get.stan.pval <- function(fit, coefName)
{
    ...
    list(mu.pval=mu.pval, mu.pval2=mu.pval2, mu.pval.Fn=mu.pval.Fn, mu=mu, mu.bar=mu.bar)
}
